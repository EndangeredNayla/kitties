var GameState = function() {
	...
	this.start = function() {
			...
			buttons.forEach(function(btn) {
				$(btn).on("click", function() {
					...
					if ($(this).hasClass("active")) {
						gameState.combo = $(this).attr("id") == "twokind-button" ? 2 : 3;
					} else {
						gameState.combo = 0;
					}
				});
		...
		(function(gameState) {
			$("#mute-button").on("click", function() {
				if (gameState.assets["ogg/atomic.ogg"].muted) {
					gameState.assets["ogg/atomic.ogg"].muted = false;
					$("#mute-button").text("Muted Game");
				} else {
					gameState.assets["ogg/atomic.ogg"].muted = true;
					$("#mute-button").text("Unmute Game");
				}
			});
		})(this);
		...
		this.console("<span style='color:yellow'>Welcome to Exploding Kitties!</span>");
	...
	this.readFromServer = function(ev) {
		...
		if (parts[0] == "hand") {
			...
			for (var i=1; i < parts.length; i++) {
				var card = $("<img class='card' src='assets/img/cards/card_"+parts[i]+".png' />");
		...
		if (ev.data == "draw_pile yes") {
			$("#draw-pile").html("<img class='card' src='assets/img/cards/card_back.png' />");
		...
		if (parts[0] == "now_playing") {
			...
			if (parts[1] == this.name) {
				this.ourTurn = true;
				this.assets["ogg/atomic.ogg"].play();
		...
		if (parts[0] == "drew_other") {
			...
			// Animation
			$("#draw-pile-animation").html("<img src='assets/img/cards/card_back.png' class='card' />");
		...
		if (parts[0] == "exploded") {
			let encoded = entities(parts[1]);
			this.console("<span style='color:purple'>"+encoded+" drew a Exploding Kitten!</span>");
		...
		if (parts[0] == "played") {
			...
			$("#discard-pile").html("<img class='card' src='assets/img/cards/card_"+parts[2]+".png' />");
		...
		if (parts[0] == "played_multiple") {
			...
			$("#discard-pile").html("<img class='card' src='assets/img/cards/card_"+parts[3]+".png' />");
